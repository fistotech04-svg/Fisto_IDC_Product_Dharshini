...
const MainEditor = () => {
  ...
  const [flipbookSettings, setFlipbookSettings] = useState(() => getRestoredState('flipbookSettings', null));
              ...
              try {
                  ...
                  if (res.data.pages && res.data.pages.length > 0) {
                       ...
                       if (res.data.settings) {
                           setFlipbookSettings(res.data.settings);
                       }
  ...
  const openPreview = useCallback(() => {
    // Sync settings from localStorage (from CustomizedEditor)
    const branding = JSON.parse(localStorage.getItem('customized_editor_branding') || '{}');
    const appearance = JSON.parse(localStorage.getItem('customized_editor_appearance') || '{}');
    const setup = JSON.parse(localStorage.getItem('customized_editor_setup') || '{}');

    setFlipbookSettings(prev => ({
        ...prev,
        logo: branding.logo || prev?.logo,
        profile: branding.profile || prev?.profile,
        background: appearance.background || prev?.background,
        appearance: appearance.appearance || prev?.appearance,
        menubar: setup.menuBar || prev?.menubar,
        othersetup: setup.otherSetup || prev?.othersetup,
        leadform: setup.leadForm || prev?.leadform,
        visibility: setup.visibility || prev?.visibility
    }));
          ...
          >
            ...
            >
                <HTMLTemplateEditor
                    ...
                    bookAppearanceSettings={flipbookSettings?.appearance}
                /> 
