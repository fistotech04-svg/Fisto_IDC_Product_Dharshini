...
const HTMLTemplateEditor = forwardRef(({
  ...
  const { cornerRadius: rawRadius = '0px' } = React.useMemo(() => 
    BookAppearanceHelpers.processBookAppearanceSettings(bookAppearanceSettings),
    [bookAppearanceSettings]
  );
  
  const cornerRadius = rawRadius || '0px';
         ...
         const writeContent = (content) => {
             ...
             internalHtmlRefs.current[p.index] = content;
              setTimeout(() => {
                ...
                initSlideshowRunner(doc);
                initAnimationRunner(doc);
              }, 100);
         };
                ...
                <div 
                  ...
                  className={`bg-white relative z-10`}
                    style={{
                      ...
                      borderRadius: (() => {
                        if (!isDoublePage) return cornerRadius;
                        
                        // Spread mode logic
                        if (p.index === 0) return `0 ${cornerRadius} ${cornerRadius} 0`; // Front Cover (Right side)
                        
                        const isLastPage = p.index === pages.length - 1;
                        if (isLastPage && pages.length % 2 === 0) return `${cornerRadius} 0 0 ${cornerRadius}`; // Back Cover if even (Left side)
                        
                        // Regular spread pages
                        return p.index % 2 !== 0 
                          ? `${cornerRadius} 0 0 ${cornerRadius}` // Left page
                          : `0 ${cornerRadius} ${cornerRadius} 0`; // Right page
                      })(),
                      overflow: 'hidden'
                    }}
                  onClick={() => !p.isEditable && onPageChange(p.index)}
                >
                  {/* Texture Overlay - Scaled for editor */}
                  {(() => {
                    const texture = BookAppearanceHelpers.processBookAppearanceSettings(bookAppearanceSettings).textureStyle;
                    if (!texture) return null;
                    
                    const finalBorderRadius = !isDoublePage ? cornerRadius : (
                      p.index === 0 ? `0 ${cornerRadius} ${cornerRadius} 0` :
                      (p.index === pages.length - 1 && pages.length % 2 === 0) ? `${cornerRadius} 0 0 ${cornerRadius}` :
                      (p.index % 2 !== 0 ? `${cornerRadius} 0 0 ${cornerRadius}` : `0 ${cornerRadius} ${cornerRadius} 0`)
                    );
